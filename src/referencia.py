...
def rodar_host(dh1, dh2, logDir):
  ...
  c2 = dh1.docker.containers.run(
        ...
        command="iperf3 -i 1 -t %d -c %s -p 5201 -J --logfile /mnt/log/iperf3_tcp.json"
          % (config.iperfTestDuration, dh2.ipAddr),
  ...
  c2 = dh1.docker.containers.run(
        ...
        command="sockperf pp -i %s -t %d --mps=100 --tcp -p 11111 --full-rtt --full-log /mnt/log/sockperf_pp_tcp.csv"
          % (dh2.ipAddr, config.sockperfTestDuration),
        volumes={logDir: {'bind': '/mnt/log', 'mode': 'rw'}},
        detach=True)
...
def rodar_bridge_cfg1(dh1, dh2, logDir):
  ...
  c1_inspect = dh2.docker.api.inspect_container(c1.id)
...
def rodar_macvlan(dh1, dh2, logDir):
  #############################################################################
  # iPerf TCP
  timePrint("iPerf TCP [STARTING]")
  c1 = dh2.docker.containers.run(
  ...
  c2 = dh1.docker.containers.run(
        ...
        volumes={logDir: {'bind': '/mnt/log', 'mode': 'rw'}},
        detach=True)
  c2_inspect = dh1.docker.api.inspect_container(c2.id)
  c2_ip = c2_inspect['NetworkSettings']['Networks'][config.nwName_macvlan]['IPAddress']
  print(c2_ip)
...
def rodar_overlay(dh1, dh2, logDir):
  #############################################################################
  # iPerf TCP
  timePrint("iPerf TCP [STARTING]")
  c1 = dh2.docker.containers.run(
